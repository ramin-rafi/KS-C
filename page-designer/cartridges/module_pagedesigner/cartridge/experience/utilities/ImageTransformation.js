 ...
 * @param {Object} teaser DIS configuration
 */
ImageTransformation.scale = function (metaData, device, teaser) {
    var transformObj = null;
    if (teaser && teaser[device]) {
        transformObj = teaser[device];
        transformObj.format = 'jpg';
...
ImageTransformation.url = function (image, options) {
    ...
    //     transform = ImageTransformation.scale(image.metaData, options.device, options.teaser);
    // }
